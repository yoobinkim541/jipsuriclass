...
## 2026-06-17 - 메인 간편 자가진단(#symptoms) UI 리프레시 + 분야별 색상 코딩

Changed files:
- `src/styles.css`, `src/App.tsx`

Implemented behavior:
- 메인 페이지 `SymptomsSection`(#symptoms)을 더 깔끔·모던·직관적으로.
  - 카테고리 카드: radius 16px, 부드러운 그림자 + 호버 리프트.
  - 이모지를 라운드 "아이콘 타일"로 → 스캔성↑. 카드 헤더 아래 구분선 추가.
  - 증상 칩: 풀폭 행 + 우측 "→" 셰브론(평소 옅게, 호버 시 슬라이드) → 클릭 가능함이 직관적.
  - 모바일 카테고리 그리드도 radius/그림자로 톤 통일.
- **분야별 색상 코딩(2차 개선)**: 카드/모바일칩에 `data-cat` 추가, 분야별 `--cat-accent`(문=탄, 누수=블루, 벽=클레이, 전기=앰버, 욕실=틸, 주방=그린)를 정의해 상속. 아이콘 타일 배경/링, 카드 호버 보더·그림자, 증상칩 호버·셰브론, 모바일 좌측 액센트 바에 `color-mix`로 적용 → 분야를 색으로 즉시 구분(직관성·모던함↑).
- 다크모드: 라벨 텍스트 navy→--ink, 아이콘 타일은 `color-mix(accent, --panel-strong)`로 톤 다운. 구분선·링은 var(--border)가 다크에서 하어라인 자동 전환.

Verification:
- `npm run build`(astro) 통과(color-mix 포함). Playwright로 #symptoms 라이트/다크/모바일 실제 렌더 캡처 육안 확인 — 색상 코딩·아이콘 타일·구분선·셰브론·모바일 액센트 바·다크 대비 모두 정상.
