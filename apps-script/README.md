...
2. `QuoteSheet.gs` 내용을 붙여넣고, 상단 설정값을 채움
   - `TEMPLATE_ID`, `DEST_FOLDER_ID`
   - **비밀키는 소스에 박지 않습니다.** `톱니바퀴(프로젝트 설정) → 스크립트 속성 → 속성 추가`
     에서 키 `SECRET` / 값 = 임의의 긴 문자열을 등록하세요. (미설정 시 웹앱이 모든 요청을 거부)
   - `CELL` 셀 주소를 **실제 템플릿 양식에 맞게** 조정 (고객/견적대상/상세내역 앵커/합계/계약금·잔금/메모)
3. **배포 → 새 배포 → 유형: 웹 앱**
   - 실행: **나**, 액세스: **모든 사용자**
4. 배포 URL(`https://script.google.com/macros/s/.../exec`) 복사

> 보안: 고객명·주소·항목명 등 문자열은 `=`/`+`/`-`/`@`로 시작하면 작은따옴표를 붙여 **텍스트로 강제**합니다(수식 주입 방지). 코드 수정 후에는 **반드시 새 배포로 갱신**해야 적용됩니다.

## 3) 사이트 환경변수 (Vercel)
| 변수 | 값 |
| --- | --- |
| `QUOTE_SHEET_WEBAPP_URL` | 위 웹앱 배포 URL (…/exec) |
| `QUOTE_SHEET_SECRET` | 스크립트 속성 `SECRET`과 동일 |
